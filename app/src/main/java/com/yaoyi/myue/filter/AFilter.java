...
public abstract class AFilter {
     ...
     * 单位矩阵
     */
    public static final float[] OM= MatrixUtils.getOriginalMatrix();       //4阶单位矩阵
    /**
     * 程序句柄
     */
    ...
    //顶点坐标 //顶点描述屏幕上待画的部分
    private float pos[] = {
        -1.0f,  1.0f,//绘制方向确定，好戏那个是三角绘制
        -1.0f, -1.0f,
        1.0f, 1.0f,
        1.0f,  -1.0f,
    };

    //纹理坐标  先绘制好的，然后一定规则投到屏幕的顶点坐标上
    private float[] coord={//纹理坐标为矩形图片、因此是四个点
        0.0f, 0.0f,         //配置应该是4*32bit
        0.0f,  1.0f,
        1.0f,  0.0f,
        1.0f, 1.0f,
    };
    ...
    public void draw(){
        onClear();//清画布，不需要传参数进来就可完成
        onUseProgram();//运行编译好的gl程序
        onSetExpandData();//设置扩展数据，？？不确定是否要扩展参数
        onBindTexture();//绑定默认纹理，
        onDraw();
    }
    ...
    protected void onUseProgram(){
        GLES20.glUseProgram(mProgram);//运行程序
    }
    ...
    protected void onDraw(){
        GLES20.glEnableVertexAttribArray(mHPosition);//用四个点确定屏幕上渲染区域
        GLES20.glVertexAttribPointer(mHPosition,2, GLES20.GL_FLOAT, false, 0,mVerBuffer);
        GLES20.glEnableVertexAttribArray(mHCoord);//送四个点确定纹理中要被画到屏幕上的区域
        GLES20.glVertexAttribPointer(mHCoord, 2, GLES20.GL_FLOAT, false, 0, mTexBuffer);
        GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP,0,4);//画四个点
    ...
    protected void onClear(){
        GLES20.glClearColor(1.0f, 1.0f, 1.0f, 1.0f);//白色且透明
        GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT | GLES20.GL_DEPTH_BUFFER_BIT);//清除屏幕颜色和深度
    }
    ...
    protected void onSetExpandData(){
        GLES20.glUniformMatrix4fv(mHMatrix,1,false,matrix,0);//将矩阵传递到程序里面去，
        //其中mMartix为gl程序中已编译变量的索引，用martix来改变它
    }
    ...
    protected void onBindTexture(){
        GLES20.glActiveTexture(GLES20.GL_TEXTURE0+textureType);//绑定前要激活某一个纹理单元，最多32个纹理，0-31
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D,getTextureId()); //用getTextureId获取外部纹理，然后绑定到上面激活的单元
        GLES20.glUniform1i(mHTexture,textureType);//告诉gl程序从上面激活的纹理单元去采样纹理
    }
