...
public enum EasyGlUtils {
    ...
    public static void genTexturesWithParameter(int size,int[] textures,int start,
        ...
        for (int i = 0; i < size; i++) {
            ...
            //绑定GL_TEXTURE_2D纹理到textures0
            GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0,gl_format, width, height,
                0, gl_format, GLES20.GL_UNSIGNED_BYTE, null);
            //生成GL_TEXTURE_2D纹理
            useTexParameter();
        }
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D,0);//取消GL_TEXTURE_2D纹理与textures0的绑定
    }

    public static void bindFrameTexture(int frameBufferId,int textureId){
        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, frameBufferId);//绑定Gl的帧buffer到frameBufferId
        GLES20.glFramebufferTexture2D(GLES20.GL_FRAMEBUFFER, GLES20.GL_COLOR_ATTACHMENT0,
            GLES20.GL_TEXTURE_2D, textureId, 0);//指定帧buffer和纹理对应
    }
