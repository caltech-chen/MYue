package com.yaoyi.myue.gpufilter;
...
import com.yaoyi.myue.gpufilter.helper.MagicFilterFactory;
...
public class SlideGpuFilterGroup {
    ...
    private void drawSlideRight(int textureId) {
        GLES20.glViewport(0, 0, width, height);//定义opengles 窗口大小
        GLES20.glEnable(GLES20.GL_SCISSOR_TEST);//启用剪裁测试
        GLES20.glScissor(0, 0, width - offset, height);//窗口被清理的大小变了 以0 0为起点，后面对应宽高
        curFilter.onDrawFrame(textureId);
        GLES20.glDisable(GLES20.GL_SCISSOR_TEST);
        GLES20.glViewport(0, 0, width, height);
        GLES20.glEnable(GLES20.GL_SCISSOR_TEST);
