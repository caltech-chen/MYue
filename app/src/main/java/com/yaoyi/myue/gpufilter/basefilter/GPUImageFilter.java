...
package com.yaoyi.myue.gpufilter.basefilter;

import android.graphics.PointF;
import android.opengl.GLES20;

import com.yaoyi.myue.gpufilter.utils.OpenGlUtils;
import com.yaoyi.myue.gpufilter.utils.Rotation;
import com.yaoyi.myue.gpufilter.utils.TextureRotationUtil;
...
public class GPUImageFilter {
    ...
    public int onDrawFrame(final int textureId) {
		...
		GLES20.glVertexAttribPointer(mGLAttribPosition, 2, GLES20.GL_FLOAT,
                false, 0, mGLCubeBuffer);//
        //顶点位置信息、一个顶点由2个值组成、浮点、不要将该数据类型标准化、数组中每个元素的长度、数组首地址
		GLES20.glEnableVertexAttribArray(mGLAttribPosition);//开启顶点属性数组
		...
		if (textureId != OpenGlUtils.NO_TEXTURE) {
            GLES20.glActiveTexture(GLES20.GL_TEXTURE0);
		    GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureId);
		    GLES20.glUniform1i(mGLUniformTexture, 0);//对应第一层纹理
		}
		onDrawArraysPre();
		GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, 4);
		//将顶点数组用三角形绘制，0表示绘制第一值的位置，4表示数组长度
		GLES20.glDisableVertexAttribArray(mGLAttribPosition);
		GLES20.glDisableVertexAttribArray(mGLAttribTextureCoordinate);
		onDrawArraysAfter();
		GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, 0);
